package gambi.zerone.camscanner.functions
...
import gambi.zerone.camscanner.helpers.await
import gambi.zerone.camscanner.helpers.calcResizedDimensions
import gambi.zerone.camscanner.helpers.getImageBounds
import gambi.zerone.camscanner.helpers.largestDimension
import gambi.zerone.camscanner.helpers.rememberCameraProvider
...
open class ImageProvider : BaseImageProvider() {
	...
	private val bmpFlowTemp = MutableSharedFlow<Pair<Bitmap, Int>>(
		replay = 1, onBufferOverflow = BufferOverflow.DROP_OLDEST
	)

	override val bmpFlow: SharedFlow<Pair<Bitmap, Int>> = bmpFlowTemp
...
fun ShowImage(
	...
	Canvas(modifier = modifier) {
		val bmp = currentBitmap ?: return@Canvas
		val imageBitmap = bmp.asImageBitmap()
		val rotatedBounds = bmp.getImageBounds(currentRotation)
		val scale = min(
			size.height / rotatedBounds.rotatedHeight, size.width / rotatedBounds.rotatedWidth
		)

		withTransform({
			translate(
				left = (size.width - imageBitmap.width * scale) / 2f,
				top = (size.height - imageBitmap.height * scale) / 2f
			)
			rotate(
				degrees = currentRotation.toFloat(), pivot = Offset(
					imageBitmap.width * scale / 2f, imageBitmap.height * scale / 2f
				)
			)
			scale(
				scaleX = scale, scaleY = scale, pivot = Offset.Zero
			)
		}) {
			drawImage(
				image = imageBitmap,
...
private fun fullDraw(
	canvas: Canvas, view: View, bitmap: Bitmap, rotation: Int, filter: Boolean = false
) {
...
//        if (bitmap.width < canvas.maximumBitmapWidth && bitmap.height <= canvas.maximumBitmapHeight) bitmap
		if (bitmap.width < 3000 && bitmap.height <= 3000) bitmap
			...
			val newDimensions = calcResizedDimensions(
				bitmap.height, bitmap.width, canvas.largestDimension.toDouble()
			)
			bitmap.scale(
				newDimensions.calculatedWidth.toInt(), newDimensions.calculatedHeight.toInt(), true
			)
		}
...
@Composable
fun ImagePlayer(
	modifier: Modifier = Modifier, imageProvider: BaseImageProvider
) {
...
//    }
	...
	var surfaceChanged by remember { mutableIntStateOf(0) }
...
fun Canvas.drawScaledBitmap(
	surface: View, bitmap: Bitmap, rotation: Int = 0, filter: Boolean = false
) {
	...
	val mScale = min(
		surface.height.toFloat() / bounds.rotatedHeight,
		surface.width.toFloat() / bounds.rotatedWidth
	)
...
fun ImageAnalyser(
	...
	LaunchedEffect(null) {
		...
		val useCases = kotlin.collections.ArrayList<UseCase>()
		...
		cameraProvider.bindToLifecycle(customImageOwner, cameraSelector, *useCases.toTypedArray())
	}
